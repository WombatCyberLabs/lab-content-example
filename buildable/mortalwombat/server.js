...
io.on('connection', socket => {
	...
	socket.on('update',wombat =>{
		...
		if(hackDetection(wombat)){
			...
			delete gameIndexs[socket.id];
			//gameIndexs = gameIndexs.filter(i=>(i !=socket.id && i!=game[gid]));
			otherPlayer.emit('disconnected');
			socket.emit('hacked','lab{hp_hacked}');
		} else
	...
	socket.on('end',(flip,won)=>{
		if(won) socket.emit('won','lab{fight_won}');
		let game = games[gameIndexs[socket.id]];
...
let hackDetection = (wombat)=>{
	if(wombat.hp >= 200 || wombat.maxhp >= 200)
		return true;
	return false;
}
let port = 8000
